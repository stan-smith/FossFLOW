...
export const Connector: ModeActions = {
  ...
  mousemove: ({ uiState, scene }) => {
    ...
    if (
      ...
      const connectorItem = (scene.currentView.connectors ?? []).find((c) => {
        return c.id === connectorMode.id;
      });
    ...
    if (uiState.connectorInteractionMode === 'click') {
      ...
      if (!uiState.mode.startAnchor) {
        ...
        if (currentMode.id) {
          ...
          const connector = (scene.currentView.connectors ?? []).find((c) => {
            return c.id === currentMode.id;
          });
