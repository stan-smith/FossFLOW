      ...
      view.connectors
        ?.map((connector) => {
          const fromIndex = items.findIndex((item) => {
            return item.id === connector.anchors[0]?.ref.item;
          });
...
export const transformFromCompactFormat = (compactModel: any): Model => {
  ...
  const fullIcons = Array.from(iconSet).map((iconName) => {
    ...
    const existingIcon = availableIcons.find((icon) => {
      return icon.id === iconName || icon.name === iconName;
    });
